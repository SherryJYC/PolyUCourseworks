...
static volatile char key_values[] = {'4'};
...
static int zili_demo_char_button_open(struct inode *inode, struct file *file)
{
	...
	irq = gpio_to_irq(KEY5);
	ret = request_irq(irq, zili_demo_char_button_interrupt, IRQ_TYPE_EDGE_BOTH, "KEY5", NULL);
	if(ret != 0)
	{
		printk("Fail to Register IRQ for KEY5\n");
	}
	
	irq = gpio_to_irq(KEY6);
	ret = request_irq(irq, zili_demo_char_button_interrupt, IRQ_TYPE_EDGE_BOTH, "KEY6", NULL);
	if(ret != 0)
	{
		printk("Fail to Register IRQ for KEY6\n");
	}

	irq = gpio_to_irq(KEY7);
	ret = request_irq(irq, zili_demo_char_button_interrupt, IRQ_TYPE_EDGE_BOTH, "KEY7", NULL);
	if(ret != 0)
	{
		printk("Fail to Register IRQ for KEY7\n");
	}
	irq = gpio_to_irq(KEY8);
	ret = request_irq(irq, zili_demo_char_button_interrupt, IRQ_TYPE_EDGE_BOTH, "KEY8", NULL);
	if(ret != 0)
	{
		printk("Fail to Register IRQ for KEY8\n");
	}
