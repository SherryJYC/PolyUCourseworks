...
int main() {
	...
	while (fgets(line, 1000, fp) != NULL) {
		...
		if (idxfp != NULL) {
			...
			while (fread(&c, 1, 1, idxfp) > 0) {
				if (isalpha(c) != 0) {
					...
					if (StopList.find(str) == StopList.end()) {
						...
						sprintf(sql, "insert into post values(\"%s\", %ld, %d)", str, fcnt, count++);
						printf("%s\n", sql);
			...
			// insert file info into table doclen
			sprintf(sql, "insert into doclen(docid, name, wordcount, path) values(%d, \"%s\", %d, \"%s\")", fcnt++, id, count, file);
			if (mysql_query(&InvFile.conn, sql)) {
				printf("error:%s\n", mysql_error(&InvFile.conn));
			}
		}
